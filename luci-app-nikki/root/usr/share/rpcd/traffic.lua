...
function traffic_stats()
    ...
    -- 调用 ucode 脚本获取统计数据
    local result = sys.exec("/usr/bin/ucode /etc/nikki/ucode/traffic-api.uc stats " .. period .. " " .. date)
    
    if result and result ~= "" then
        local stats = util.json_decode(result)
        if stats then
            -- 获取 IP 统计数据
            local ip_result = sys.exec("/usr/bin/ucode /etc/nikki/ucode/traffic-api.uc ip-stats " .. date)
